// Hand-maintained: edit this file directly. Regeneration does not update this
// file.
//
// If the file is deleted, Speakeasy scaffolds this file as a TODO-error stub
// `since x-speakeasy-plan-validators: "RequestConfigInputValidator"` annotation
// in overlay.yaml has no corresponding implementation on disk. Once a real
// implementation is committed, speakeasy stops touching the file — it is not
// tracked in .speakeasy/gen.lock and `speakeasy run` will not regenerate it
// while it exists. OpenAPI can't express the business rules below
// (override-flag consistency, allowed-groups population, approver shape),
// so they must live as Go code here.
